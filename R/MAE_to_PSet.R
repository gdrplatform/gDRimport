...
convert_MAE_to_PSet <- function(mae,
    ...
    # checkmate::assert_named(mae, "assays")
    # checkmate::assert_named(mae, "colData")
    # checkmate::assert_named(mae, "rowData")
    # checkmate::assert_named(mae, "metadata")
    ...
    assay_list <- lapply(assay_names, function(x){
        ...
        # Combine all the data.tables into one data.table, add the name of the Summarized Experiment as a "data_type" column
        # so that users can identify which Summarized Experiment the data came from
    ...
    rowdata_ <- data.table::rbindlist(gDRutils::MAEpply(mae, FUN=function(x) data.table::as.data.table(SummarizedExperiment::rowData(x), keep.rownames=TRUE)), fill=TRUE)
    ...
    coldata_ <- unique(data.table::rbindlist(gDRutils::MAEpply(mae, FUN=function(x) data.table::as.data.table(SummarizedExperiment::colData(x), keep.rownames=TRUE)), fill=TRUE))
    # rename the rownames column to "sampleid" (required for PharmacoSet Object)
    data.table::setnames(coldata_, "rn", "sampleid")

    # Create a list of rowname and column name identifiers for each assay name (required for TreatmentResponseExperiment object)
    ...
    PharmacoGx::PharmacoSet2(
        name = pset_name, 
        treatment = as.data.frame(rowdata_), 
        sample = as.data.frame(coldata_),
        molecularProfiles = 
...
#'
.extract_or_create_assay <- function(SE, assay_name){
