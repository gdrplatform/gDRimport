...
```{r inspect-tre-rows, echo=TRUE}
tre <- treatmentResponse(pset)
head(rownames(tre))
head(rowData(tre))
```

```{r inspect-tre-cols, echo=TRUE}
head(colnames(tre))
head(colData(tre))
```
...
```{r subset-tre-data-table, echo=TRUE}
tre[
...
``` {r view-tre-assay-names}
assayNames(tre)
```


``` {r inspect-metrics-assay}
